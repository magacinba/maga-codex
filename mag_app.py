...
# OR-Tools za TSP optimizaciju
ORTOOLS_AVAILABLE = False
try:
    from ortools.constraint_solver import routing_enums_pb2
    from ortools.constraint_solver import pywrapcp
    ORTOOLS_AVAILABLE = True
    print("✅ OR-Tools uspešno uvezen")
...
def route_optimal_multistart(locations: List[str], max_starts: int = 8) -> List[str]:
    """
    Standardni algoritam sa nearest neighbor i 2-opt optimizacijom.
    Koristi se kao fallback ako OR-Tools nije dostupan.
    """
    locs = [l.strip().upper() for l in locations if l.strip()]
    if len(locs) <= 2:
        return locs
...
def route_with_ortools(locations: List[str], time_limit_seconds: int = 10) -> List[str]:
    """
    Koristi Google OR-Tools za pronalaženje najkraće putanje.
    NE pretpostavlja nikakav redosled – pusti OR-Tools da radi svoj posao.
    """
    print("🚀🚀🚀 OR-Tools funkcija je POZVANA! 🚀🚀🚀")
    print(f"🔍 OR-Tools funkcija pozvana!")
    print(f"🔍 ORTOOLS_AVAILABLE = {ORTOOLS_AVAILABLE}")
    
    locs = [l.strip().upper() for l in locations if l.strip()]
    
    if len(locs) <= 3:
        print("⚠️ Premalo lokacija za OR-Tools")
        return locs
    
    if not ORTOOLS_AVAILABLE:
        print("⚠️ OR-Tools nije dostupan, koristim fallback")
        return route_optimal_multistart(locs)
    
    print(f"✅ OR-Tools je dostupan, pokušavam da rešim TSP za {len(locs)} lokacija...")
    
    # NE SORTIRAMO LOKACIJE – ostavljamo ih u originalnom redosledu
    # Jer OR-Tools sam treba da nađe najbolji redosled
    
    # Pravimo mapu lokacija -> indeksi (prema originalnom redosledu)
    loc_to_index = {loc: i for i, loc in enumerate(locs)}
    index_to_loc = {i: loc for i, loc in enumerate(locs)}
    
    # Pravimo matricu udaljenosti
    n = len(locs)
    distance_matrix = [[0] * n for _ in range(n)]
    
    print("⏳ Računam matricu udaljenosti...")
    for i, loc_i in enumerate(locs):
        for j, loc_j in enumerate(locs):
            if i != j:
                # Računamo stvarnu udaljenost kroz tvoj WarehouseMap
                dist = WAREHOUSE.distance_between_locations(loc_i, loc_j)
                # OR-Tools radi sa integerima, množimo sa 1000 za milimetre
                distance_matrix[i][j] = int(dist * 1000)
            else:
                distance_matrix[i][j] = 0
    
    # Definiši strategije za prvo rešenje (od najbolje do najbrže)
    ...
    for strategy_idx, strategy in enumerate(first_solution_strategies):
        ...
        print(f"⏳ Pokrećem strategiju {strategy_idx+1}/{len(first_solution_strategies)}: {strategy_name}")
        
        # Kreiranje modela
        manager = pywrapcp.RoutingIndexManager(n, 1, 0)  # 1 vozilo, polazak iz prve tačke
        routing = pywrapcp.RoutingModel(manager)
        
        # Callback za udaljenosti
        ...
        search_parameters.time_limit.seconds = max(1, time_limit_seconds // len(first_solution_strategies))
    ...
    if best_route:
        print(f"✅✅✅ OR-Tools pronašao rutu od {best_distance:.1f}m ✅✅✅")
        print(f"   Prvih 5 lokacija: {best_route[:5]}")
        return best_route
    
    # Ako ništa ne uspe, vrati se na standardni algoritam
    print("⚠️ OR-Tools ne radi, koristim fallback algoritam")
    return route_optimal_multistart(locs)
...
def compute_route(locations: List[str], mode: str) -> List[str]:
    ...
    print(f"\n🔍 compute_route: mode='{mode}', lokacija={len(locations)}")
    print(f"🔍 ORTOOLS_AVAILABLE = {ORTOOLS_AVAILABLE}")
    ...
    if mode == "sector":
        ...
        return route_with_ortools(locations)
    elif mode == "optimal":
        if ORTOOLS_AVAILABLE and len(locations) > 5:
            print("✅ OR-Tools dostupan, koristim ga za optimal")
            return route_with_ortools(locations)
        else:
            print("⚠️ OR-Tools nije dostupan, koristim standardni")
            return route_optimal_multistart(locations)
    else:
        print(f"⚠️ Nepoznat mode '{mode}', koristim standardni")
        return route_optimal_multistart(locations)
