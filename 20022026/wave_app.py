...
def update_wave_item(session_id: str, req: WaveUpdateRequest):
    ...
    for it in arr:
        if it["sku"] == sku and it["invoice"] == req.invoice:
            ...
            if action == "take":
                ...
                it["note"] = note
                it["updated_at"] = now
                log_action("WAVE_UPDATE", f"TAKE {sku} {req.invoice}@{cur_loc}", {"picked": picked})
    ...
    if not found:
        raise HTTPException(status_code=400, detail=f"SKU {sku} za fakturu {req.invoice} ne postoji na lokaciji {cur_loc}.")
