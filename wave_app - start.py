    ...
    for it in arr:
        if it["sku"] == sku and it["invoice"] == req.invoice:
            ...
            if action == "take":
                ...
                # NE menjamo status, ostaje pending
                # NE menjamo qty_picked
                log_action("WAVE_UPDATE", f"PROBLEM {sku} {req.invoice}@{target_loc}", {"note": note})
                ...
                it["note"] = (it["note"] or "") + " | " + (note or "Naknadna dopuna")
                it["updated_at"] = now
                log_action("WAVE_UPDATE", f"DOPUNA {sku} {req.invoice}@{target_loc}", {"picked": req.qty_picked})
